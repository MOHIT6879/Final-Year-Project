...
def _get_prediction(
    ...
    return return_objs,emotion_index_dict[prediction_index]
...
def predict(
    ...
    if webcam_mode:
        ...
        result,label = _get_prediction(best_hp=best_hyperparameters, img=image, model=model, imshow=imshow, video_mode=video_mode, verbose=verbose, grad_cam=grad_cam)
        ...
        if verbose:
            ...
            for res in result:
                ...
                if type(res) == np.ndarray:
                    ...
                    with open("face_output.txt", "w") as file: # error
                        file.write(res)
                    print("written to the file") ## POTENTIAL ERROR POSSIBLE
