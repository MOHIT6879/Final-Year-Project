...
if __name__ == "__main__":
    ...
    # if no args are passed take inp from the user 
    if len(sys.argv) <= 1:
    ...
    # if args are passed extract the first arg that represents the audio or face or combined model 
    else:
        ch = int(sys.argv[1])
    ...
    if ch == 1:
        ...
        # checking if any extra args are passed 
        if len(sys.argv) <= 2:
            ch = int(input("1) Tune hyperparameters\n"
                           ...
                           "5) Test on test set\n"
                           "\n\nEnter your choice: "))
        # extract the arg from the cmd line args 
        else:
            ch = int(sys.argv[2])
